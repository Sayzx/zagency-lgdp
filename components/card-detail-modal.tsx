...
export function CardDetailModal() {
  ...
  const handleAddLabel = async (labelId: string) => {
    try {
      ...
      const labelIds = updatedCard.labels?.map((l: any) => {
  ...
  const handleAddAssignee = async (userId: string) => {
    try {
      ...
      const userIds = updatedCard.assignedTo?.map((u: any) => {
  ...
  return (
    <Dialog open={!!selectedCardId} onOpenChange={handleClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] w-[90vw] bg-zinc-900 border-zinc-800 text-white p-0 flex flex-col">
        <div className="flex h-full overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-6 flex flex-col">
            ...
            <div className="mt-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium text-zinc-400">Labels</span>
              </div>
            ...
            <div className="mt-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-medium text-zinc-400">Comments ({(card.comments ?? []).length})</span>
              </div>
                        ...
                        <div key={comment.id} className="flex gap-2">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="text-sm font-medium text-white">{commentUser?.name}</span>
                              <span className="text-xs text-zinc-500">
                                {format(new Date(comment.createdAt), "MMM d 'at' h:mm a")}
                              </span>
                            </div>
                            <p className="text-sm text-zinc-300 bg-zinc-800 p-2 rounded break-words">{comment.content}</p>
                          </div>
                        </div>
                      )
                    })}
            ...
            <div className="mt-6">
                    ...
                    <div className="flex items-center justify-center gap-2 text-zinc-400">
                      {isUploadingAttachment ? (
                        ...
                        <>
                          ...
                          <span className="text-sm">Click to upload files or images</span>
                        </>
                      )}
                    </div>
                  </div>
              ...
              {card.attachments && card.attachments.length > 0 && (
                <div className="mb-4">
                  <p className="text-xs text-zinc-500 mb-2">Saved Attachments</p>
                  <div className="space-y-2">
                    {card.attachments.map((attachment) => (
                      <div key={attachment.id} className="flex items-center justify-between p-2 bg-zinc-800 rounded border border-zinc-700">
                ...
                {uploadedFiles.length > 0 && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {uploadedFiles.map((file, index) => (
                      <div key={index} className="flex items-center justify-between p-2 bg-zinc-800 rounded opacity-60">
                        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          {/* Sidebar */}
          <ScrollArea className="w-80 border-l border-zinc-800 bg-zinc-900/50">
            <div className="space-y-4 p-4">
              ...
              <div>
                <div className="flex items-center justify-between gap-2 mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-zinc-400">Assigned To</span>
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  {availableUsers.length > 0 ? (
                    <Select onValueChange={handleAddAssignee}>
                      ...
                      <SelectContent className="bg-zinc-900 border-zinc-800">
                        {availableUsers.map((user) => {
                          const userName = user.name || `${user.firstName || ""} ${user.lastName || ""}`.trim() || user.email || user.username || "Unknown"
                          return (
                    ...
                    <div className="p-2 text-xs text-zinc-500 bg-zinc-800 rounded text-center">
                      {currentProject?.members?.length === 0 ? "No members in project" : "All members assigned"}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <Select value={(card.priority || "").toLowerCase()} onValueChange={(value) => handleUpdatePriority(value.toUpperCase() as Priority)}>
                  ...
                  <SelectContent className="bg-zinc-900 border-zinc-800">
                    <SelectItem value="urgent" className="text-zinc-200">
                      <div className="flex items-center gap-2">
                        <div className={`h-2 w-2 rounded-full ${priorityColors.urgent}`} />
                        Urgent
                      </div>
                    </SelectItem>
      ...
      <Dialog open={!!previewAttachment} onOpenChange={(open) => !open && setPreviewAttachment(null)}>
        <DialogContent className="max-w-2xl bg-zinc-900 border-zinc-800 p-4">
          <DialogHeader>
            <DialogTitle className="text-white">{previewAttachment?.name}</DialogTitle>
          </DialogHeader>
          {previewAttachment && (
            <div className="flex flex-col items-center justify-center gap-4">
              ...
              />
              <p className="mt-4 text-xs text-zinc-400">{(previewAttachment.size / 1024).toFixed(2)} KB</p>
            </div>
          )}
        </DialogContent>
      </Dialog>
